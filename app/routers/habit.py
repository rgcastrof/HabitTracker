...
from app.routers.utils import db_users, db_habits
...
async def get_habit(user_id: int):
...
async def update_habit(habit_id: int, habit: HabitUpdate):
...
async def get_habits(page: int = Query(1, ge=1), page_size: int = Query(5, ge=1)):
